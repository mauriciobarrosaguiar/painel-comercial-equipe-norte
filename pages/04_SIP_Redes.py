...
if st.button("Cadastrar nova SIP", use_container_width=True):
...
s1, s2 = st.columns(2)
if s1.button("Salvar SIP", use_container_width=True, disabled=not nome_final or not cnpjs):
    adicionar_sip(
        nome=nome_final,
        redes=redes_sel,
        cnpjs=cnpjs,
...
if editando:
    ...
    if s2.button("Excluir SIP", use_container_width=True, disabled=not confirmar):
...
if not grupos:
    ...
    st.dataframe(pedidos_visual, use_container_width=True, height=360)
