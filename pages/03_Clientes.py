...
if filtrado.empty:
    ...
    if nav1.button("Voltar", use_container_width=True, disabled=st.session_state[chave_pagina] <= 1):
    ...
    if nav3.button("Próxima", use_container_width=True, disabled=st.session_state[chave_pagina] >= total_paginas):
        st.session_state[chave_pagina] += 1
        st.rerun()
