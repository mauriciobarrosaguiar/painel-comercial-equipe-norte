...
def registrar_upload(chave: str, arquivo) -> None:
    if arquivo is None:
        return
    _uploads_sessao()[chave] = {"name": arquivo.name, "bytes": arquivo.getvalue()}
    st.cache_data.clear()
...
def fonte_ativa(chave: str) -> str:
    ...
    if upload:
        return f"Upload da sessao: {upload.get('name', '')}"
    caminho = ARQUIVOS_PADRAO[chave]
    return f"Pasta data: {caminho.name}" if caminho.exists() else "Arquivo nao encontrado"
...
def carregar_dados_tratados() -> dict[str, pd.DataFrame | list[str]]:
    ...
    if acoes_raw.empty:
        avisos.append("template_acoes_promocionais.xlsx: sem acoes cadastradas. Use a tela Importar Bases para baixar o modelo.")
