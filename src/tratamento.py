...
COLUNAS_ACOES = COLUNAS_ACOES_BASE + COLUNAS_ACOES_META
...
def normalizar_tipo_meta_unidades(valor: object) -> str:
    texto = normalizar_texto_alto(valor)
    termos_individuais = ["CADA", "INDIVIDUAL", "POR PRODUTO", "POR MOLECULA", "POR SKU"]
    if any(termo in texto for termo in termos_individuais):
        return "POR_PRODUTO"
    return "SOMANDO"


def normalizar_escopo_meta(valor: object) -> str:
    texto = normalizar_texto_alto(valor)
    if any(termo in texto for termo in ["CONSULTOR", "INDIVIDUAL", "SEPARAD"]):
        return "POR_CONSULTOR"
    return "PADRAO"
...
def preparar_acoes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    aliases = {
        ...
        "status": ["STATUS"],
        "meta_unidades": ["META UNIDADES", "META QUANTIDADE", "OBJETIVO UNIDADES", "OBJETIVO", "QTD MINIMA", "QTD_MINIMA"],
        "meta_cnpjs": ["META CNPJS", "META CNPJ", "META PDVS", "META PDV", "CNPJS", "PDVS"],
        "tipo_meta_unidades": [
            "TIPO META UNIDADES",
            "TIPO_META_UNIDADES",
            "MODO META",
        ...
        "escopo_meta": ["ESCOPO META", "ESCOPO_META", "TIPO META", "META CONSULTORES"],
        "meta_consultores": ["METAS CONSULTORES", "METAS_CONSULTORES", "META POR CONSULTOR", "META_POR_CONSULTOR"],
    }
    for destino, nomes in aliases.items():
        base = renomear_alias(base, destino, nomes)
    ...
    base["ean_limpo"] = base["ean"].apply(normalizar_ean)
    base["ean"] = base["ean_limpo"]
    for coluna in ["campanha", "produto", "distribuidora", "consultor", "observacao", "status", "meta_consultores"]:
        base[coluna] = base[coluna].apply(normalizar_texto)
    base["tipo_mix"] = base["tipo_mix"].apply(normalizar_tipo_mix)
    base["desconto"] = base["desconto"].apply(converter_numero)
    base["meta_unidades"] = base["meta_unidades"].apply(converter_numero)
    base["meta_cnpjs"] = base["meta_cnpjs"].apply(converter_numero)
    escopo_bruto = base["escopo_meta"].copy()
    base["tipo_meta_unidades"] = base["tipo_meta_unidades"].apply(normalizar_tipo_meta_unidades)
    base["escopo_meta"] = base["escopo_meta"].apply(normalizar_escopo_meta)
    base.loc[
        escopo_bruto.apply(normalizar_texto).eq("") & base["meta_consultores"].ne(""),
        "escopo_meta",
    ] = "POR_CONSULTOR"
