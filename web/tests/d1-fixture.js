...
function statement(db,sql,params=[]){return{bind(...values){return statement(db,sql,values)},async all(){return{results:db.prepare(sql).all(...params)}},async first(){return db.prepare(sql).get(...params)||null},async run(){const r=db.prepare(sql).run(...params);return{success:true,meta:{changes:Number(r.changes||0)}}}}}
