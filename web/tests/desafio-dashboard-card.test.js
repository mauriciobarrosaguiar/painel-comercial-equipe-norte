...
import { MAURICIO_TEST_ID, personalDatabase, testDatabase } from './d1-fixture.js'
...
const historyHelper = readFileSync(new URL('../functions/_lib/desafio-gigantes-historico.js', import.meta.url), 'utf8')
const historyMigration = readFileSync(new URL('../migrations/10024_desafio_gigantes_historico.sql', import.meta.url), 'utf8')
const historyUi = readFileSync(new URL('../src/DesafioGigantesHistory.tsx', import.meta.url), 'utf8')
const historyModule = readFileSync(new URL('../src/HistoryModule.tsx', import.meta.url), 'utf8')
const closeWorkflow = readFileSync(new URL('../../.github/workflows/fechamento-mensal.yml', import.meta.url), 'utf8')

async function inserirMeta(DB, { id='dg1', escopo='consultor', consultor=MAURICIO_TEST_ID, nome='MAURICIO BARROS DE AGUIAR', setor='18150301', sku='10018', ean='111', metaPos=1, metaGiro=1 }={}) {
...
async function carregar(DB, query=`ano_mes=2026-07&consultor=${MAURICIO_TEST_ID}`) {
...
test('quantidade atendida sem faturamento não positiva PDV nem aumenta Giro', async () => {
  const DB = await personalDatabase()
  await inserirMeta(DB)
  await DB.prepare("INSERT INTO pedidos VALUES('p-extra','PX','NFX','cl2',?,'2026-07-14','2026-07-14','FATURADO',0,'BUSSOLA',1,'2026-07-14')").bind(MAURICIO_TEST_ID).run()
...
test('resultado pessoal ignora vendas de outro consultor', async () => {
  const DB = await personalDatabase()
  await inserirMeta(DB)
  ...
  const body = await carregar(DB)
  assert.equal(body.escopo, 'consultor')
  assert.equal(body.pos_80, 1)
  assert.equal(body.giro_bruto_100, 1)
  assert.equal(body.giro_100, 1)
  assert.equal(body.pontuacao_estimada, 200)
...
test('modo de fechamento devolve resultado final de cada SKU para congelamento histórico', async () => {
  const DB = await personalDatabase()
  await inserirMeta(DB)
  const normal = await carregar(DB)
  assert.equal(Object.hasOwn(normal, 'produtos'), false)
  const detalhado = await carregar(DB, `ano_mes=2026-07&consultor=${MAURICIO_TEST_ID}&detalhes=1`)
  assert.equal(detalhado.produtos.length, 1)
  assert.equal(detalhado.produtos[0].sku, '10018')
  assert.equal(detalhado.produtos[0].positivacao_real, 1)
  assert.equal(detalhado.produtos[0].giro_real, 1)
  assert.equal(detalhado.produtos[0].pontos_estimados, 200)
})

test('fechamento mensal congela o Desafio e meses seguintes não reescrevem o mês fechado', () => {
  assert.match(historyMigration, /UNIQUE\(ano_mes, escopo, referencia_id\)/)
  assert.match(historyHelper, /INSERT OR IGNORE INTO desafio_gigantes_fechamentos/)
  assert.match(historyHelper, /if \(existente\)/)
  assert.match(historyHelper, /preservados \+= 1/)
  assert.match(historyHelper, /produtos_resultado/)
  assert.match(historyHelper, /regras: resumo\.regras/)
  assert.match(closeWorkflow, /Congelar resultado do Desafio de Gigantes/)
  assert.match(closeWorkflow, /api\/internal\/desafio-gigantes-fechamento/)
  assert.match(historyModule, /DesafioGigantesHistory anoMes=\{selected\}/)
  assert.match(historyUi, /Produtos, metas, regras e resultados dos meses seguintes não alteram esta fotografia/)
  assert.match(historyUi, /Resultado final por produto/)
})
