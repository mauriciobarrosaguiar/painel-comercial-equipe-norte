...
test('mês fechado pessoal recebe faturamento retroativo sem alterar as metas congeladas', async () => {
  ...
  await DB.prepare(`
    ...
    ) VALUES('mc',?,'consultor','2026-07',600,180,120,2,0,'imp','2026-07-01')
  `).bind(MAURICIO_TEST_ID).run()
  ...
  const consultorInicial = initialHistory.itens.find((item) => item.escopo === 'CONSULTOR' && item.referencia_id === MAURICIO_TEST_ID)
  ...
  assert.equal(initialHistory.geral[0].resultado.meta_ol_sem_combate, 600)
  ...
  assert.ok(consultorAtualizado, 'Histórico pessoal do Maurício deve existir após a reconciliação')
