...
test('Mercado Farma usa buybox, busca de PDV e Excel dinâmico por UF', () => {
  const market = read('src/MarketFarmaModule.v2.tsx')
  const workflow = read('../.github/workflows/mercadofarma.yml')
  const download = read('functions/api/mercado-farma-excel-v2.js')
  ...
  assert.match(market, /Digite parte do nome do PDV ou CNPJ/)
  assert.match(market, /cliente_cnpj/)
  ...
  assert.match(workflow, /Sincronizar consolidado no Turso/)
})
