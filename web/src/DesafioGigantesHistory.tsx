...
export default function DesafioGigantesHistory({anoMes}:{anoMes:string}){
  ...
  const card=(item:Item)=>{const r=item.snapshot?.resumo||{},max=Number(r.maximo_estimado||0),score=Number(r.pontuacao_estimada||0),reach=max>0?score/max*100:0;return <article className="dgh-card" key={item.id}><div><span>{item.escopo==='gerente'?'GD / Gerente':'Consultor'}</span><strong>{item.referencia_nome}</strong></div><b>{num.format(score)} pts</b><small>{pct.format(reach)}% do máximo · fechado em {dateLabel(item.fechado_em)}</small><div className="dgh-stats"><span>SKUs <b>{Number(r.skus||0)}</b></span><span>Pos. ≥80% <b>{Number(r.pos_80||0)}</b></span><span>Giro ≥100% <b>{Number(r.giro_bruto_100||0)}</b></span><span>Giro pontuando <b>{Number(r.giro_100||0)}</b></span></div><details><summary>Produtos e metas congelados</summary><div className="dgh-products">{(item.snapshot?.produtos_meta||[]).map(p=><div key={`${item.id}-${p.sku}`}><strong>{p.produto}</strong><small>SAP {p.sku} · EAN {p.ean||'não identificado'} · Meta Pos. {num.format(p.meta_positivacao)} · Meta Giro {num.format(p.meta_giro)}</small></div>)}</div></details></article>}
  return <section className="dgh-history"><div className="operations-heading"><div><span className="eyebrow">Campanha mensal congelada</span><h2>🏆 Desafio de Gigantes</h2><p>O fechamento deste mês é permanente. Produtos, metas e resultados dos meses seguintes não alteram esta fotografia.</p></div><span>{data.itens.length} fechamento(s)</span></div>{error&&<div className="alert alert-error">{error}</div>}{loading?<div className="operations-empty">Carregando fechamento do Desafio…</div>:<><div className="dgh-manager">{manager&&card(manager)}</div><div className="dgh-grid">{consultants.map(card)}</div></>}</section>
}
