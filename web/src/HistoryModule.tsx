...
export default function HistoryModule({ onBack, onAutomations }: { onBack: () => void; onAutomations: () => void }) {
  ...
  const metricas = [['OL total', 'ol_total'], ['OL sem combate', 'ol_sem_combate'], ['Prioritários', 'ol_prioritarios'], ['Lançamentos', 'ol_lancamentos']]
  ...
  return <main className="content operations-page">
    ...
    <section className="operations-hero">
      <div><span className="eyebrow">Fotografias permanentes</span><h1>Histórico mensal</h1><p>Resultados fechados e histórico de compras por mês de cada cliente.</p></div>
    ...
    {mesImportado && <div className="alert alert-success">Mês carregado da planilha histórica. O OL total, pedidos, clientes e positivação estão disponíveis; o mix de produtos depende do fechamento mensal do painel.</div>}
    {!selected && !loading ? <div className="history-empty"><h2>Nenhum mês fechado</h2><p>Anexe os meses anteriores ou faça o primeiro fechamento.</p></div> : <>
      <section className="history-summary">
        {metricas.map(([l, k]) => <article key={k}><span>{l}</span><strong>{loading ? '—' : money.format(valor(atual, k))}</strong><small className={variacao(k) < 0 ? 'variation-negative' : 'variation-positive'}>{variacao(k) >= 0 ? '+' : ''}{pct.format(variacao(k))}% versus {mes(anterior?.ano_mes || 'mês anterior')}</small></article>)}
        <article><span>Clientes com venda</span><strong>{num.format(valor(atual, 'clientes_com_venda'))}</strong><small>{num.format(valor(atual, 'clientes_sem_venda'))} sem venda · {pct.format(valor(atual, 'positivacao_percentual'))}%</small></article>
        <article><span>Resultado da meta</span><strong>{pct.format(valor(atual, 'resultado_meta_ol'))}%</strong><small>Meta: {money.format(valor(atual, 'meta_ol_sem_combate'))}</small></article>
      </section>
