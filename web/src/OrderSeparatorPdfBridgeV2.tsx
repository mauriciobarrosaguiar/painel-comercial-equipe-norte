...
type ExtractedRow = { ean: string; quantidade: string; descricao: string; codigoInterno: string }
...
function parseRows(items: PositionedItem[], header: HeaderInfo, width: number) {
  const anchors = items
    .filter(item => item.y < header.y - 1 && /^\d{12,14}$/.test(digits(item.text)))
  ...
  for (let index = 0; index < anchors.length; index += 1) {
    ...
    const lower = next ? next.y + 2 : current.y - Math.max(11, Math.min(26, typicalGap - 1))
...
async function readPdf(file: File): Promise<PdfResult> {
  ...
  for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber += 1) {
    ...
    const header = findHeader(items, Number(viewport.width || 595))
    if (header) {
      ...
      rows.push(...parseRows(items, header, Number(viewport.width || 595)))
    } else {
      rows.push(...parseFallback(items, Number(viewport.width || 595)))
  ...
  if (!cnpjs.length) warnings.push('CNPJ não identificado automaticamente. Digite o CNPJ do cliente.')
  if (cnpjs.length > 1) warnings.push('Mais de um CNPJ encontrado. Selecione o CNPJ do cliente, e não o do fornecedor.')
...
function PdfPortal() {
  ...
  function confirm() {
    if (!parsed) return
    if (digits(cnpj).length !== 14) { setMessage('Informe um CNPJ válido do cliente para continuar.'); return }
    const standardized = parsed.rows.map(row => [
      formatCnpj(cnpj), unidade, uf.toUpperCase().slice(0, 2), clean(row[eanField]), clean(row[productField]), clean(row[qtyField]),
  ...
  return createPortal(
    <article className="separator-upload-card" style={{ gridColumn: '1 / -1' }}>
      ...
      {parsed && <section style={{ marginTop: 22, textAlign: 'left', borderTop: '1px solid #dce6ed', paddingTop: 18 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(180px,1fr))', gap: 12, marginTop: 14 }}>
          <label><b>CNPJ do cliente</b>
            ...
            <input style={{ width: '100%', minHeight: 42, marginTop: 6, boxSizing: 'border-box' }} value={cnpj} onChange={event => setCnpj(event.target.value)} placeholder="Digite o CNPJ" />
          </label>
          ...
          <label><b>UF</b><input style={{ width: '100%', minHeight: 42, marginTop: 6, boxSizing: 'border-box' }} maxLength={2} value={uf} onChange={event => setUf(event.target.value.toUpperCase().slice(0, 2))} /></label>
        </div>
        ...
        <div className="separator-table-wrap"><table><thead><tr><th>EAN</th><th>Produto</th><th>Quantidade</th></tr></thead><tbody>
          {preview.map((row, index) => <tr key={`${row.ean}-${index}`}><td>{clean(row[eanField]) || '—'}</td><td>{clean(row[productField]) || '—'}</td><td>{clean(row[qtyField]) || '—'}</td></tr>)}
        </tbody></table></div>
