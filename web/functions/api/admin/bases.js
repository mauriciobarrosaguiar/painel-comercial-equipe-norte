...
async function executarJson(env,sql,rows,tamanho=400){
  for(let i=0;i<rows.length;i+=tamanho){
    await env.DB.prepare(sql).bind(JSON.stringify(rows.slice(i,i+tamanho))).run()
  }
}

async function registrar(env,tipo,nome,total,status='concluido'){
  const id=`imp-${crypto.randomUUID()}`,agora=new Date().toISOString()
  await env.DB.prepare('INSERT INTO importacoes(id,tipo,nome_arquivo,total_registros,status,criado_em) VALUES(?,?,?,?,?,?)').bind(id,tipo,nome,total,status,agora).run()
}
...
async function status(env){
  ...
  return{
    painel:Number(r[0]?.results?.[0]?.total||0),produtos_mix:Number(r[1]?.results?.[0]?.total||0),produtos_mercado_farma:Number(r[2]?.results?.[0]?.total||0),metas:Number(r[3]?.results?.[0]?.total||0),historico:r[4]?.results||[]
  }
}
...
async function importarPainel(env,rows,nome){
  ...
  for(const row of rows){
    const cnpj=digitos(row.cnpj).slice(-14).padStart(14,'0')
    ...
    clientes.push({
      id:await idEstavel('cli',cnpj),cnpj,nome_fantasia:texto(row.nome_pdv||row.nome_fantasia||row.razao_social),cidade:texto(row.cidade),uf:alto(row.uf).slice(0,2),situacao:texto(row.situacao),grupo_economico:texto(row.grupo_economico),rede_associacao:texto(row.rede_associacao),bandeira:texto(row.bandeira),nome_gd:texto(row.nome_gd),consultor_id:consultorId,setor_rep:texto(row.setor_rep),foco_pex:texto(row.foco_pex),positivacao:texto(row.positivacao),grupo_sip:texto(row.grupo_economico||row.rede_associacao||row.bandeira||row.nome_pdv),ativo:ativo(row.situacao)?1:0,token
    })
  }
  if(!clientes.length)throw new Error('O Painel Equipe Norte não possui clientes válidos.')
  const agora=new Date().toISOString()
  await executarJson(env,`INSERT INTO consultores(id,nome,origem,ativo,atualizado_em) SELECT json_extract(value,'$.id'),json_extract(value,'$.nome'),'PAINEL_EQUIPE',1,? FROM json_each(?) WHERE 1 ON CONFLICT(id) DO UPDATE SET nome=excluded.nome,origem='PAINEL_EQUIPE',ativo=1,atualizado_em=excluded.atualizado_em`,[...consultores.values()].map(x=>({...x,agora})).map(x=>x),400).catch(async()=>{
...
async function importarProdutos(env,rows,nome,tipo){
  ...
  if(tipo==='produtos_mix'){
    await executarJson(env,`INSERT INTO produtos(id,ean,descricao,tipo_mix,ativo,mix_importacao_id,atualizado_em) SELECT json_extract(value,'$.id'),json_extract(value,'$.ean'),json_extract(value,'$.descricao'),json_extract(value,'$.tipo_mix'),1,json_extract(value,'$.token'),? FROM json_each(?) WHERE 1 ON CONFLICT(ean) DO UPDATE SET descricao=excluded.descricao,tipo_mix=excluded.tipo_mix,ativo=1,mix_importacao_id=excluded.mix_importacao_id,atualizado_em=excluded.atualizado_em`,dados,400)
    await env.DB.prepare("UPDATE produtos SET tipo_mix='SEM CLASSIFICACAO' WHERE COALESCE(mix_importacao_id,'')<>?").bind(token).run()
  }else{
    await executarJson(env,`INSERT INTO produtos(id,ean,descricao,ativo,mercado_farma_ativo,mercado_farma_importacao_id,atualizado_em) SELECT json_extract(value,'$.id'),json_extract(value,'$.ean'),json_extract(value,'$.descricao'),1,1,json_extract(value,'$.token'),? FROM json_each(?) WHERE 1 ON CONFLICT(ean) DO UPDATE SET descricao=excluded.descricao,ativo=1,mercado_farma_ativo=1,mercado_farma_importacao_id=excluded.mercado_farma_importacao_id,atualizado_em=excluded.atualizado_em`,dados,400)
...
async function importarMetas(env,rows,nome,anoMes){
  ...
  const dados=[]
  for(const row of rows){const nomeConsultor=texto(row.consultor||row.colaborador);if(!nomeConsultor)continue;const consultorId=await idEstavel('cons',nomeConsultor);dados.push({id:await idEstavel('meta',anoMes,'consultor',consultorId),consultor_id:consultorId,nome:nomeConsultor,ol_sem_combate:numero(row.ol_sem_combate),ol_prioritarios:numero(row.ol_prioritarios),ol_lancamentos:numero(row.ol_lancamentos),clientes_positivados:numero(row.clientes_positivados)})}
...
export async function onRequestGet({request,env}){const negado=await admin(request,env);if(negado)return negado;return json(await status(env))}
export async function onRequestPost({request,env}){
  ...
  try{
    ...
    let resultado
    if(tipo==='painel')resultado=await importarPainel(env,rows,nome)
    else if(tipo==='metas')resultado=await importarMetas(env,rows,nome,texto(body.ano_mes))
    else resultado=await importarProdutos(env,rows,nome,tipo)
    return json({sucesso:true,tipo,...resultado,bases:await status(env)})
