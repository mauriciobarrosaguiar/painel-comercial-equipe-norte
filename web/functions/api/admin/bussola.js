...
export async function onRequestPost({ request, env }) {
  ...
  await env.DB.prepare(
    ...
    masked,
    encrypted,
    'Credenciais salvas com criptografia. Valide a configuração antes da primeira extração.',
    now,
  ).run()
  ...
  return json({
    ...
    status: 'configurada',
    mensagem: 'Credenciais salvas com segurança.',
    atualizado_em: now,
  })
}
...
export async function onRequestDelete({ request, env }) {
  ...
  return json({ sucesso: true, mensagem: 'Credenciais removidas.' })
}
