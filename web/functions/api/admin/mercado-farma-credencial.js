...
async function requireAdmin(request, env) {
  if (!secretReady(env)) return json({ erro: 'A chave administrativa ainda não foi configurada.' }, 503)
...
export async function onRequestGet({ request, env }) {
  ...
  return json({
    ...
    mensagem: current?.mensagem_status || 'Seu acesso pessoal do Mercado Farma ainda não foi cadastrado.',
...
export async function onRequestPost({ request, env }) {
  ...
  const encrypted = await encryptCredentials({ usuario, segredo, salvo_em: new Date().toISOString() }, env.PAINEL_ADMIN_KEY)
  ...
  ).bind(INTEGRATION, masked, encrypted, 'Acesso pessoal do Mercado Farma salvo com criptografia.', now).run()

  return json({ sucesso:true, configurada:true, usuario_mascarado:masked, status:'configurada', mensagem:'Acesso pessoal salvo com segurança.', atualizado_em:now })
}
...
export async function onRequestDelete({ request, env }) {
  ...
  return json({ sucesso:true, mensagem:'Acesso pessoal do Mercado Farma removido.' })
}
