...
function worksheet({ cliente, consultor, itens, totais }) {
  const lines = [
    ...
    `<row r="5">${stringCell('A5', 'Nome do Consultor:', 2)}${stringCell('B5', consultor, 3)}</row>`,
    `<row r="7" ht="24" customHeight="1">${['PRODUTO','EAN','QNTDE','DISTRIBUIDORA ESCOLHIDA'].map((label,index) => stringCell(`${String.fromCharCode(65+index)}7`,label,1)).join('')}</row>`,
  ]
  ...
  const summaryStart = 9 + itens.length
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:D${totalRow}"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="7" topLeftCell="A8" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews>
  ...
  <sheetData>${lines.join('')}</sheetData><mergeCells count="5"><mergeCell ref="A1:D1"/><mergeCell ref="B3:D3"/><mergeCell ref="B4:D4"/><mergeCell ref="B5:D5"/><mergeCell ref="A${summaryStart}:D${summaryStart}"/></mergeCells>
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const solicitados = (Array.isArray(body?.itens) ? body.itens : []).map(item => ({
      ean: texto(item?.ean).replace(/\D/g, ''),
      quantidade: Math.max(0, Math.floor(numero(item?.quantidade))),
      distribuidora: texto(item?.distribuidora),
      uf: texto(item?.uf).toUpperCase().slice(0, 2),
    ...
    const consultas = solicitados.map(item => env.DB.prepare(`
      ...
      LIMIT 1
    `).bind(item.ean, item.distribuidora, item.uf))
    ...
    const itens = solicitados.map((solicitado, index) => {
      ...
      if (!oferta) throw new Error(`A oferta do EAN ${solicitado.ean} não está mais disponível. Atualize o Mercado Farma.`)
      const valorUnitario = numero(oferta.preco_com_imposto) || numero(oferta.preco_sem_imposto)
      return {
        produto: texto(oferta.produto),
        ean: texto(oferta.ean).replace(/\D/g, ''),
        quantidade: solicitado.quantidade,
        distribuidora: texto(oferta.distribuidora),
        distribuidora_rotulo: `${texto(oferta.distribuidora)}${oferta.uf ? ` - ${texto(oferta.uf)}` : ''}`,
    ...
    const bytes = buildXlsx({
      sheetName: 'Pedido',
      title: `Pedido Mercado Farma - ${cliente.nome}`,
      worksheetXml: worksheet({ cliente, consultor: session.nome, itens, totais }),
      stylesXml: styles(),
    })
    ...
    return new Response(`Não foi possível gerar o pedido em Excel. ${error instanceof Error ? error.message : String(error)}`, {
      status: 500,
