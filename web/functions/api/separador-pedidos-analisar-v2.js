...
const validCnpj = value => {
  ...
  return cnpj.length === 14 && cnpj !== '00000000000000' ? cnpj : ''
}
  ...
  for (const eanGroup of chunks(eans, 70)) {
    ...
    for (const item of result.results || []) {
      ...
      if (!exists) map.get(key).push({
        produto: text(item.produto), distribuidora: text(item.distribuidora), estoque: numeric(item.estoque),
        preco: numeric(item.preco_sem_imposto), atualizado_em: item.atualizado_em || null,
      })
    }
  }
  return map
...
function summarize(groups) {
  ...
  let distributed = 0, noStock = 0, notFound = 0, minFailed = 0
  for (const group of groups) {
    const distributorRows = [...group.allocation.totals.entries()].map(([distribuidora, total]) => {
      ...
      return {
        ...
        total: Number(total.toFixed(2)), pedido_minimo: minimum,
    ...
    cnpjRows.push({ cnpj: group.label, uf: group.uf, modo: group.allocation.mode, distribuidoras: distributorRows, ...counts })
    ...
    state.cnpjs += 1; state.produtos += group.items.length; state.distribuidos += counts.distribuidos
    state.sem_estoque += counts.sem_estoque; state.ean_nao_localizado += counts.ean_nao_localizado; state.minimo_nao_atingido += counts.minimo_nao_atingido
  }
  return {
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const mapping = {
      cnpj: safeIndex(body?.mapping?.cnpj), ean: safeIndex(body?.mapping?.ean), produto: safeIndex(body?.mapping?.produto),
      quantidade: safeIndex(body?.mapping?.quantidade), unidade: safeIndex(body?.mapping?.unidade), uf: safeIndex(body?.mapping?.uf),
    }
    if (!headers.length || !sourceRows.length || mapping.ean < 0 || mapping.quantidade < 0) {
      return json({ erro: 'Informe uma planilha válida e confirme as colunas de EAN e quantidade.' }, 400)
    }

    const config = sanitizeConfig(body?.configuracao || {})
    const prepared = sourceRows.map((row, index) => {
      const cnpj = mapping.cnpj >= 0 ? validCnpj(valueAt(row, mapping.cnpj)) : ''
      const ean = cleanEan(valueAt(row, mapping.ean))
      const quantidade = Math.max(0, numeric(valueAt(row, mapping.quantidade)))
      const produto = text(valueAt(row, mapping.produto))
      const unidade = text(valueAt(row, mapping.unidade))
      const fallbackUf = text(valueAt(row, mapping.uf)).toUpperCase().slice(0, 2) || inferUf(unidade) || text(body?.uf_selecionada_no_painel).toUpperCase().slice(0, 2)
      return { index, row, cnpj, ean, quantidade, produto, unidade, fallbackUf }
    })

    const validCnpjs = [...new Set(prepared.map(item => item.cnpj).filter(Boolean))]
    ...
    for (const item of prepared) {
      const label = item.cnpj || (item.unidade ? `SEM CNPJ · ${item.unidade}` : 'SEM CNPJ')
      if (!item.uf || !config.estados[item.uf]) {
        invalidResults.push({ index: item.index, cnpj: label, uf: item.uf || '', ean: item.ean, produto: item.produto, quantidade: item.quantidade, distribuidora: 'UF SEM CONFIGURAÇÃO', status: 'UF SEM CONFIGURAÇÃO', preco_sem_imposto: null, total_linha: null })
        continue
      }
      if (!item.ean || item.quantidade <= 0) {
        invalidResults.push({ index: item.index, cnpj: label, uf: item.uf, ean: item.ean, produto: item.produto, quantidade: item.quantidade, distribuidora: 'DADOS INVÁLIDOS', status: 'DADOS INVÁLIDOS', preco_sem_imposto: null, total_linha: null })
        continue
      }
      const identity = item.cnpj || `SEM-CNPJ:${item.unidade || 'PEDIDO'}`
      const key = `${item.uf}|${identity}`
      if (!groupMap.has(key)) groupMap.set(key, { cnpj: item.cnpj, label, uf: item.uf, items: [] })
    ...
    for (const group of groups) {
      for (const item of group.items) {
        results.push({
          index: item.index, cnpj: group.label, uf: item.uf, ean: item.ean,
          produto: item.produto || item.offer?.produto || item.offers?.[0]?.produto || '', quantidade: item.quantidade,
          distribuidora: item.offer?.distribuidora || item.status, status: item.status,
