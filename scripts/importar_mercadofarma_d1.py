...
def requisicao(method: str, url: str, *, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ultimo_erro = ""
    for tentativa in range(1, 6):
        ...
        if not resposta.ok or not dados.get("success", False):
            ...
            raise RuntimeError(f"Falha na API Cloudflare: HTTP {resposta.status_code} - {erros or dados}")
        return dados
...
def localizar_database_id() -> str:
    ...
    banco = next((item for item in bancos if str(item.get("name", "")) == DATABASE_NAME), None)
...
def executar(database_id: str, sql: str, params: Iterable[Any] | None = None) -> dict[str, Any]:
    payload = {"sql": sql, "params": ["" if valor is None else str(valor) for valor in (params or [])]}
...
def executar_lotes(database_id: str, consultas: list[dict[str, Any]], tamanho: int = 5) -> None:
    url = f"{API_BASE}/accounts/{ACCOUNT_ID}/d1/database/{database_id}/query"
    for inicio in range(0, len(consultas), tamanho):
        ...
        payload = {
            "batch": [
                {
                    "sql": item["sql"],
                    "params": ["" if valor is None else str(valor) for valor in item.get("params", [])],
                }
                for item in bloco
            ]
        }
...
def carregar_base() -> pd.DataFrame:
    ...
    base = pd.read_csv(CSV_PATH, encoding="utf-8-sig", dtype=str, keep_default_na=False)
    ...
    if faltantes:
        raise RuntimeError("Colunas obrigatórias ausentes: " + ", ".join(faltantes))
    ...
    base = base[(base["UF"] != "") & (base["EAN"] != "") & (base["DISTRIBUIDORA"] != "")].copy()
    base = base.drop_duplicates(subset=["UF", "EAN", "DISTRIBUIDORA"], keep="last").reset_index(drop=True)
    if base.empty:
        raise RuntimeError("O consolidado do Mercado Farma não contém registros válidos.")
    return base


def consulta_multiplos_valores(tabela: str, colunas: list[str], linhas: list[list[Any]], conflito: str) -> dict[str, Any]:
    placeholders = "(" + ",".join("?" for _ in colunas) + ")"
    sql = f"INSERT INTO {tabela} ({','.join(colunas)}) VALUES " + ",".join(placeholders for _ in linhas) + " " + conflito
    params = [valor for linha in linhas for valor in linha]
...
def sincronizar() -> None:
    ...
    executar(
        database_id,
        "INSERT INTO extracoes (id,tipo,status,solicitado_por,github_run_id,iniciado_em,criado_em) VALUES (?,?,?,?,?,?,?)",
        [
            extracao_id,
            "MERCADO_FARMA",
            "executando",
    ...
    try:
        executar(
            ...
            CREATE INDEX IF NOT EXISTS idx_mf_staging_run ON mercado_farma_precos_staging(run_id);
            """,
        )
        executar(database_id, "DELETE FROM mercado_farma_precos_staging WHERE atualizado_em < datetime('now','-2 days')")

        produtos = base[["EAN", "PRODUTO"]].drop_duplicates(subset=["EAN"], keep="last")
        consultas_produtos: list[dict[str, Any]] = []
        linhas_produtos: list[list[Any]] = []
        for item in produtos.itertuples(index=False):
            ean = texto(item.EAN)
            linhas_produtos.append([id_estavel("prod", ean), ean, texto(item.PRODUTO), "EMS Genéricos", "SEM CLASSIFICACAO", 1, timestamp])
            if len(linhas_produtos) == 100:
                consultas_produtos.append(
                    consulta_multiplos_valores(
                        "produtos",
                        ["id", "ean", "descricao", "laboratorio", "tipo_mix", "ativo", "atualizado_em"],
                        linhas_produtos,
                        "ON CONFLICT(ean) DO UPDATE SET descricao=excluded.descricao,laboratorio=excluded.laboratorio,ativo=1,atualizado_em=excluded.atualizado_em",
                    )
                )
                linhas_produtos = []
        if linhas_produtos:
        ...
        consultas_staging: list[dict[str, Any]] = []
        ...
        for item in base.itertuples(index=False):
            ...
            if len(linhas_staging) == 50:
                consultas_staging.append(
                    consulta_multiplos_valores(
        ...
        executar(
            ...
            INSERT INTO mercado_farma_precos (
              id,uf,cnpj_referencia,produto_id,ean,produto,distribuidora,estoque,desconto,
              pf_distribuidora,pf_fabrica,preco_com_imposto,preco_sem_imposto,status,erro,atualizado_em
            )
            SELECT id,uf,cnpj_referencia,produto_id,ean,produto,distribuidora,estoque,desconto,
                   pf_distribuidora,pf_fabrica,preco_com_imposto,preco_sem_imposto,status,erro,atualizado_em
        ...
        executar(
            ...
            DELETE FROM mercado_farma_precos
             WHERE uf IN ({marcadores_uf})
               AND NOT EXISTS (
                 SELECT 1 FROM mercado_farma_precos_staging s
                  WHERE s.run_id = ?
        ...
        executar(database_id, "DELETE FROM mercado_farma_precos_staging WHERE run_id = ?", [run_uuid])

        resumo = json.dumps(
            {"registros": int(len(base)), "ufs": ufs, "sincronizado_em": timestamp},
            ensure_ascii=False,
        )
        executar(
            database_id,
        ...
        executar(
            ...
            [len(base), f"Mercado Farma sincronizado para {', '.join(ufs)}", timestamp, extracao_id],
        )
        print(f"Mercado Farma sincronizado com sucesso: {len(base)} registros, UFs {', '.join(ufs)}.")
    except Exception as exc:
        try:
            executar(
                database_id,
                "UPDATE extracoes SET status='erro',erro=?,finalizado_em=? WHERE id=?",
                [str(exc)[:2000], pd.Timestamp.now(tz="America/Sao_Paulo").isoformat(), extracao_id],
            )
        except Exception:
            pass
        raise
